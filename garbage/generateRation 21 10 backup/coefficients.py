...
def get_coefs_by_nutrient(values, nutrient_name):
    ...
    # if answer < -2000:
    #     print(f'{nutrient_name}: {values} --> {answer}')
...
def get_coefs_for_dataframe(df):
    ...
    #     print(f'{col}: {df[col].values} -> {get_coefs_by_nutrient(df[col].values, col)}')
...
def df_dot(dfs, vectors):
    
    if type(dfs) != list:
        return pd.DataFrame(vectors[np.newaxis, :].dot(dfs.values), columns = dfs.columns)
    
    df = pd.concat(dfs)
    vector = np.concatenate(vectors)
    
    return pd.DataFrame(vector[np.newaxis, :].dot(df.values), columns = df.columns)
            

def get_coef_by_counts(df, vector, goal):
        
    new_df = df_dot(df, vector)
    
    return get_coefs_depended_on_goal(new_df, goal)[0]
